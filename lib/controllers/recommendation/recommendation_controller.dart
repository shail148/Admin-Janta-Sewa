...
class RecommendationController extends GetxController {
  // ---------------- Source / UI lists ----------------
  final RxList<RecommendationModel> recommendationList =
      <RecommendationModel>[].obs;
  final RxList<RecommendationModel> filteredList = <RecommendationModel>[].obs;
  ...
  Future<void> loadRecommendations() async {
    final raw = await rootBundle.loadString(
      'assets/data/recommendation_letters.json',
    );
    final List<dynamic> list = jsonDecode(raw) as List<dynamic>;
    final data = list.map((e) => RecommendationModel.fromJson(e)).toList();
  ...
  void apply() {
    List<RecommendationModel> results = List.from(recommendationList);

    // Search across useful fields
    final q = searchText.value.trim().toLowerCase();
    if (q.isNotEmpty) {
      results = results.where((a) {
        return a.recommendationId.toLowerCase().contains(q) ||
  ...
  // ---------------- Chart builders ----------------
  void _rebuildChartSeries(List<RecommendationModel> list) {
  ...
  void _rebuildSingleSeries(List<RecommendationModel> list) {
    final now = DateTime.now();
    final monday = now.subtract(Duration(days: (now.weekday + 6) % 7));
  ...
  int statusBadgeCount(String status) => status == 'All'
      ...
      : recommendationList.where((t) => t.status == status).length;
