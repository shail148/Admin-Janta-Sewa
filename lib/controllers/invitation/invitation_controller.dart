...
class InvitationController extends GetxController {
  // ---------------- Source / UI lists ----------------
  final RxList<InvitationModel> invitationList = <InvitationModel>[].obs;
  final RxList<InvitationModel> filteredList = <InvitationModel>[].obs;
  ...
  Future<void> loadInvitations() async {
    // Path: assets/data/invitations.json
    final raw = await rootBundle.loadString('assets/data/invitations_500.json');
    final List<dynamic> list = jsonDecode(raw) as List<dynamic>;
    final data = list
        .map((e) => InvitationModel.fromJson(e as Map<String, dynamic>))
        .toList();

    invitationList.assignAll(data);
    apply();
  }
  ...
  void apply() {
    List<InvitationModel> results = List.from(invitationList);

    // Search across useful fields
    final q = searchText.value.trim().toLowerCase();
    if (q.isNotEmpty) {
      results = results.where((a) {
        return a.invitationId.toLowerCase().contains(q) ||
            a.userId.toLowerCase().contains(q) ||
            a.invitationDetail.programmeName.toLowerCase().contains(q) ||
            a.invitationDetail.typeOfInvitation.toLowerCase().contains(q) ||
            a.invitationDetail.contactPersonName.toLowerCase().contains(q) ||
            a.locationDetail.district.toLowerCase().contains(q) ||
            a.locationDetail.villageWard.toLowerCase().contains(q) ||
  ...
  // ---------------- Chart builders ----------------
  void _rebuildChartSeries(List<InvitationModel> list) {
  ...
  void _rebuildSingleSeries(List<InvitationModel> list) {
    final now = DateTime.now();
    final monday = now.subtract(Duration(days: (now.weekday + 6) % 7));
  ...
  int statusBadgeCount(String status) => status == 'All'
      ...
      : invitationList.where((t) => t.status == status).length;
